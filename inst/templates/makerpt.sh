...
if [ -n "${R_HOME}"  ]; then 
  ...
  if [ $rc -eq 0 ]; then
    ...
    echo "Rscript not found on path, set R_HOME to the directory whwere R is installed, that is the directory ABOVE bin/Rscript"
    exit $rc
  fi
fi
...
if [ $rc -eq 0 ]; then
  ...
  thisdir=`pwd`
  echo "xelatex failed to process the generated tex file $texFileNameStem.tex, file kept in $thisdir"
  exit $rc
fi 
